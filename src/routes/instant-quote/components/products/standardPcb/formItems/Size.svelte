<script lang="ts">
	...
	$: ({ title, description, placeholder } = $lg.instantQuote.standardPcb.size);

	$: ({ length, width } = $quote.standardPcb);
	$: size = (length * width) / 1000000;

	$: error = $quoteError.standardPcb.length ?? $quoteError.standardPcb.width;
</script>

<FormItem {title} moreInfo={{ description, url: '' }} {error}>

<script lang="ts">
	...
	$: pd = $productDetails.standardPcb.customizedServices;
	$: ({ customizedServices } = $quote.products.standardPcb);
					...
					<input
						...
						on:click={() => {
							if (customizedServices.includes(vd.value))
								$quote.products.standardPcb.customizedServices = customizedServices.filter((v) => v !== vd.value);
							else $quote.products.standardPcb.customizedServices = [...customizedServices, vd.value];
						}}
					/>
				</FormItem>
			{/each}

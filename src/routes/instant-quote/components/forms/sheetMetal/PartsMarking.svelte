<script lang="ts">
	...
	$: l = $lg.instantQuote.sheetMetal;
	$: pd = $productDetails.sheetMetal;
	$: ({ silkScreen } = $quote.products.sheetMetal);
</script>
...
<FormItem pd={{ l: l.partMarking }}>
		...
		<div class="flex flex-wrap gap-4">
			{#each pd.silkScreen.values as { value, color }}
				<button
					...
					on:click={() => ($quote.products.sheetMetal.silkScreen = value)}
				>
					{#if value === 'NONE'}
						{$lg.common.none}
					{:else}
			...
			>
				<input
					...
					on:change={(e) => ($quote.products.sheetMetal.silkScreen = e.currentTarget.value)}
				/>
			</button>
		</div>
	</FormItem>
	<FormItem pd={{ l: l.laserEngraving }} classes="items-start">
		<input
			...
			bind:checked={$quote.products.sheetMetal.laserEngraving}
		/>
	</FormItem>
</FormItem>

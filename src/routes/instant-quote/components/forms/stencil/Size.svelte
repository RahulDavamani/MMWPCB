<script lang="ts">
	...
	$: l = $lg.instantQuote.stencil;

	$: ({ length, width } = $quote.products.stencil);
	$: size = (length * width) / 1000000;

	$: lengthError = $quoteError.stencil.length;
	$: widthError = $quoteError.stencil.width;
					...
					<div class="flex items-center gap-2">
						<label class="input input-bordered input-sm flex items-center gap-2">
							<input
								...
								on:change={(e) => ($quote.products.stencil.length = Number(e.currentTarget.value) / 0.1)}
							/>
						...
						<label class="input input-bordered input-sm flex items-center gap-2">
							<input
								...
								on:change={(e) => ($quote.products.stencil.width = Number(e.currentTarget.value) / 0.1)}
							/>
							<span class="opacity-75">inch</span>
						</label>
					</div>
					...
					<div class="flex items-center gap-2">
						<label class="input input-bordered input-sm flex items-center gap-2">
							<input
								...
								on:change={(e) => ($quote.products.stencil.length = Number(e.currentTarget.value) / 0.001)}
							/>
							<span class="opacity-75">inch</span>
						</label>
						<Icon icon="mdi:multiply" />
						<label class="input input-bordered input-sm flex items-center gap-2">
							<input
								...
								on:change={(e) => ($quote.products.stencil.width = Number(e.currentTarget.value) / 0.001)}
							/>
							<span class="opacity-75">inch</span>
						</label>
					</div>

<script lang="ts">
	...
	$: l = $lg.instantQuote.threePrinting;
	$: pd = $productDetails.threePrinting;
	$: ({ silkScreen } = $quote.products.threePrinting);
</script>
...
<FormItem pd={{ l: l.partMarking }}>
		...
		<div class="flex flex-wrap gap-4">
			{#each pd.silkScreen.values as { value, color }}
				<button
					...
					on:click={() => ($quote.products.threePrinting.silkScreen = value)}
				>
					{#if value === 'NONE'}
						{$lg.common.none}
					{:else}
			...
			>
				<input
					...
					on:change={(e) => ($quote.products.threePrinting.silkScreen = e.currentTarget.value)}
				/>
			</button>
		</div>
	</FormItem>
	<FormItem pd={{ l: l.laserEngraving }} classes="items-start">
		<input
			...
			bind:checked={$quote.products.threePrinting.laserEngraving}
		/>
	</FormItem>
</FormItem>

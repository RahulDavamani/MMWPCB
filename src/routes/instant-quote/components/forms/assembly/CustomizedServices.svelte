<script lang="ts">
	...
	$: pd = $productDetails.assembly.customizedServices;
	$: ({ customizedServices } = $quote.products.assembly);
...
<div class="collapse collapse-arrow mt-6 {collapseOpen && 'overflow-visible'}">
	...
	<div class="collapse-content">
		<div class="my-4 text-sm text-secondary">{pd.l.description}</div>
					...
					<input
						...
						on:click={() => {
							if (customizedServices.includes(vd.value))
								$quote.products.assembly.customizedServices = customizedServices.filter((v) => v !== vd.value);
							else $quote.products.assembly.customizedServices = [...customizedServices, vd.value];
						}}
					/>
				</FormItem>
			{/each}

<script lang="ts">
	...
	$: l = $lg.instantQuote.injectionMolding;
	$: pd = $productDetails.injectionMolding;
	$: ({ silkScreen } = $quote.products.injectionMolding);
</script>
...
<FormItem pd={{ l: l.partMarking }}>
		...
		<div class="flex flex-wrap gap-4">
			{#each pd.silkScreen.values as { value, color }}
				<button
					...
					on:click={() => ($quote.products.injectionMolding.silkScreen = value)}
				>
					{#if value === 'NONE'}
						{$lg.common.none}
					{:else}
			...
			>
				<input
					...
					on:change={(e) => ($quote.products.injectionMolding.silkScreen = e.currentTarget.value)}
				/>
			</button>
		</div>
	</FormItem>
	<FormItem pd={{ l: l.laserEngraving }} classes="items-start">
		<input
			...
			bind:checked={$quote.products.injectionMolding.laserEngraving}
		/>
	</FormItem>
</FormItem>

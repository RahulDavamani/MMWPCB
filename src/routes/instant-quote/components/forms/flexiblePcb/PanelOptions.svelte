<script lang="ts">
	...
	$: pd = $productDetails.flexiblePcb;
	$: ({ xoutAllowance, breakAwayRail, routeProcess, panelRequirements } = $quote.products.flexiblePcb);
</script>
			...
			{#each pd.xoutAllowance.values as { title, value }}
				<FormControl inputType="In" label={title}>
					<input
						...
						{value}
						bind:group={$quote.products.flexiblePcb.xoutAllowance}
					/>
				</FormControl>
			{/each}
		</div>
	...
	<div class="grid grid-cols-2">
		{#if routeProcess}
			<FormItem pd={pd.routeProcess}>
				<select class="select select-bordered select-sm" bind:value={$quote.products.flexiblePcb.routeProcess}>
					...
					{#each pd.breakAwayRail.values as { title, value }}
						<FormControl inputType="In" label={title}>
							<input
								...
								class="radio radio-primary radio-xs"
								{value}
								bind:group={$quote.products.flexiblePcb.breakAwayRail}
							/>
						</FormControl>
					{/each}
				</div>
...
{#if panelRequirements !== undefined && panelRequirements !== null}
	<FormItem pd={pd.panelRequirements}>
		<textarea
			...
			bind:value={$quote.products.flexiblePcb.panelRequirements}
		/>
	</FormItem>
{/if}

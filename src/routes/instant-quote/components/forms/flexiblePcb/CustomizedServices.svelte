					...
					<input
						...
						on:click={() => {
							if (customizedServices.includes(vd.value))
								$quote.products.flexiblePcb.customizedServices = customizedServices.filter((v) => v !== vd.value);
							else $quote.products.flexiblePcb.customizedServices = [...customizedServices, vd.value];
						}}
					/>
				</FormItem>
			{/each}

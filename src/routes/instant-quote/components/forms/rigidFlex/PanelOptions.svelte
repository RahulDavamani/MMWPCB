<script lang="ts">
	...
	$: pd = $productDetails.rigidFlex;
	$: ({ xoutAllowance, breakAwayRail, routeProcess, panelRequirements } = $quote.products.rigidFlex);
</script>
			...
			{#each pd.xoutAllowance.values as { title, value }}
				<FormControl inputType="In" label={title}>
					<input
						...
						{value}
						bind:group={$quote.products.rigidFlex.xoutAllowance}
					/>
				</FormControl>
			{/each}
		</div>
	...
	<div class="grid grid-cols-2">
		{#if routeProcess}
			<FormItem pd={pd.routeProcess}>
				<select class="select select-bordered select-sm" bind:value={$quote.products.rigidFlex.routeProcess}>
					...
					{#each pd.breakAwayRail.values as { title, value }}
						<FormControl inputType="In" label={title}>
							<input
								...
								class="radio radio-primary radio-xs"
								{value}
								bind:group={$quote.products.rigidFlex.breakAwayRail}
							/>
						</FormControl>
					{/each}
				</div>
...
{#if panelRequirements !== undefined && panelRequirements !== null}
	<FormItem pd={pd.panelRequirements}>
		<textarea
			...
			bind:value={$quote.products.rigidFlex.panelRequirements}
		/>
	</FormItem>
{/if}

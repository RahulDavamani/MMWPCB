<script lang="ts">
	...
	$: pd = $productDetails.advancedPcb.differentDesign;
	$: isError = $quoteError.advancedPcb.differentDesign;
	$: ({ differentDesign } = $quote.products.advancedPcb);
</script>
	...
	<div class="flex flex-wrap gap-4">
		{#each pd.values as value}
			<button
				...
				on:click={() => ($quote.products.advancedPcb.differentDesign = value)}>{value}</button
			>
		{/each}
		<button class="btn btn-sm btn-primary {differentDesign < 7 && 'btn-outline'}">
			<input
				...
				on:change={(e) => ($quote.products.advancedPcb.differentDesign = Number(e.currentTarget.value))}
			/>
		</button>
	</div>
</FormItem>

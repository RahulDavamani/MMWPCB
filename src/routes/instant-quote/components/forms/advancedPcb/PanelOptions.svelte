<script lang="ts">
	...
	$: pd = $productDetails.advancedPcb;
	$: ({ xoutAllowance, breakAwayRail, routeProcess, panelRequirements } = $quote.products.advancedPcb);
</script>
			...
			{#each pd.xoutAllowance.values as { title, value }}
				<FormControl inputType="In" label={title}>
					<input
						...
						{value}
						bind:group={$quote.products.advancedPcb.xoutAllowance}
					/>
				</FormControl>
			{/each}
		</div>
	...
	<div class="grid grid-cols-2">
		{#if routeProcess}
			<FormItem pd={pd.routeProcess}>
				<select class="select select-bordered select-sm" bind:value={$quote.products.advancedPcb.routeProcess}>
					...
					{#each pd.breakAwayRail.values as { title, value }}
						<FormControl inputType="In" label={title}>
							<input
								...
								class="radio radio-primary radio-xs"
								{value}
								bind:group={$quote.products.advancedPcb.breakAwayRail}
							/>
						</FormControl>
					{/each}
				</div>
...
{#if panelRequirements !== undefined && panelRequirements !== null}
	<FormItem pd={pd.panelRequirements}>
		<textarea
			...
			bind:value={$quote.products.advancedPcb.panelRequirements}
		/>
	</FormItem>
{/if}

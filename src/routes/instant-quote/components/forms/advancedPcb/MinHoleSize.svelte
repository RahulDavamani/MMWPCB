<script lang="ts">
	...
	$: pd = $productDetails.advancedPcb.minHoleSize;
	$: ({ minHoleSize } = $quote.products.advancedPcb);
</script>
	...
	<div class="flex flex-wrap gap-4">
		{#each pd.values as value}
			<button
				...
				on:click={() => ($quote.products.advancedPcb.minHoleSize = value)}>{pd.parseValue(value)}</button
			>
		{/each}
		<button

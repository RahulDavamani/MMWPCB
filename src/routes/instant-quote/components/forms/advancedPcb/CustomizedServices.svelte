<script lang="ts">
	...
	$: pd = $productDetails.advancedPcb.customizedServices;
	$: ({ customizedServices } = $quote.products.advancedPcb);
					...
					<input
						...
						on:click={() => {
							if (customizedServices.includes(vd.value))
								$quote.products.advancedPcb.customizedServices = customizedServices.filter((v) => v !== vd.value);
							else $quote.products.advancedPcb.customizedServices = [...customizedServices, vd.value];
						}}
					/>
				</FormItem>
			{/each}

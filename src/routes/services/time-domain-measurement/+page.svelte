<script lang="ts">
	...
	$: section = $servicesStore.timeDomainMeasurement;
	...
	$: filteredServices = services
		.filter(({ l: { title }, category, maxFrequency }) => {
...
<Layout pageTitle={l.title}>
	...
	<div class="flex flex-col md:flex-row gap-10">
		<div class="w-full md:w-1/4">
			...
			<RangeFilter title={l.specs.maxFrequency} unit="GHz" max={maxFrequency} bind:values={filters.frequency} />
		</div>
		...
		<div class="w-full mt-4">
			<SearchSort
				...
				sortBys={[
					...
					{ title: l.specs.maxFrequency, value: 'frequency' }
				]}
			/>

			<div class="grid grid-cols-1 md:grid-cols-3 gap-10 mt-6">
				{#each filteredServices as service}
					{@const { code, maxFrequency } = service}
					<ProductCard
						{service}
						href="/services/{$servicesStore.timeDomainMeasurement.code}/{code}"
						specs={[
							{
								title: l.specs.maxFrequency,
								value: `${maxFrequency}GHz`
							}
						]}
					/>
				{/each}
